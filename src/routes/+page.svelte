<script lang="ts">
  ...
  const lastBlog = $derived(content.posts[Category.BLOG].slice(0, 5));
  const moreRides: boolean = $derived(content.posts[Category.RIDES].length > lastRides.length);
  const moreBlog: boolean = $derived(content.posts[Category.BLOG].length > lastBlog.length);
...
<section class="col-span-3 col-start-1 grid grid-cols-subgrid">
  <h2
    class="
      ...
      lg:text-2xl"
  >
    <a href="blog" class="underline underline-offset-6 hover:no-underline">Блог</a>
  </h2>

  <div class="col-span-3 col-start-1 grid grid-cols-subgrid overflow-x-auto">
    <!-- Class mx-auto -->
    <ul class="col-span-2 col-start-2 flex gap-8 pr-4">
      {#each lastBlog as { slug, title, image, category, date } (slug)}
        {@const src = `https://ik.imagekit.io/sakalou/${category}/${slug}/${image}`}
        <li
          class="w-[calc(100vw_-_var(--spacing)*20)] max-w-92 shrink sm:w-[calc(50vw_-_var(--spacing)*14)]"
        >
      ...
      {#if moreBlog}
        ...
        >
          <a
            ...
            class="relative text-xl text-sky-600 underline underline-offset-4 hover:no-underline dark:text-sky-300"
          >
            <span>Другие записи в блоге →</span>
          </a>
        </li>
      {/if}
    </ul>
